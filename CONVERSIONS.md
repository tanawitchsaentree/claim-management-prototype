...
## 2026-09-02 — BMPCC-18160 ([FIGMA] FNOL — circumstance selection field + Claim / Section Overview)

- **Source:** PI 2026.4 tracker ticket, **unassigned and not started**. Built under the standing instruction given in conversation on 2026-08-28 and repeated on 2026-09-02 — *"ทำแม่งให้เสร็จ ถ้าติดก็ pupose flag ไปดิ ใส่ไปใน comment note อะไรก็ได้ที่อยู่ใน pipeline การทำงาน ควยไม่มีคน assign ก็ทำแม่งไปก่อนดิ"* — which overrides RULE -2's "no building from unassigned tickets" gate for tracker tickets with a real UI surface. Every open question is therefore recorded **in the pipeline** (in-code `ASSUMPTION [CIRC-n]` comments + tracker `blocked_note`), not asked back.
- **Module:** FNOL — Loss information · Claims — Claim Overview + Edit claim details · Sections — detail panel
- **Files touched:** 20 — new `features/fnol/config/circumstances.{ts,spec.ts}`, `shared/pipes/circumstance-label.pipe.ts`; modified `core/models/{loss-information,claim-overview}.model.ts`, `core/mock/data/{loss-information,claim-overview}.json`, `features/fnol/services/fnol-state.service.ts`, `features/fnol/steps/step-loss-information/*.{ts,html}`, `features/fnol/steps/step-summary/step-summary.component.ts`, `features/claims/claim-overview/claim-overview.component.{ts,html}`, `features/claims/edit-loss-information/{edit-loss-information.component.{ts,html},loss-info-diff.ts}`, `features/sections/{sections.ts,sections.html,section-detail-panel/*.{ts,html}}`. Commit `e493c76`.
- **Deviations:**
  - **Sections: detail panel, not a 7th table column.** The ticket says "Section Overview". The sections table already carries 6 columns after Comments and Coverage Review were added, and the value is the *claim's* — identical on every row — so a per-row column would repeat itself down the page for no information gain.
  - **Unknown is offered only when nothing narrowed the list.** Once a real peril is confirmed, the handler has to say something about it. The AC's "full list + unknown" case is read as: no cause selected, "Other event" selected, or a cause with no mapping.
- **Schema changes:** `LossInformationFormValue.circumstance: string | null` (required key, nullable value) and `LossInformation.circumstance?: string | null`; `ClaimOverview.incidentCircumstance?: string`. Keys are stored, never labels.
- **Notes:**
  - **Four genuine spec ambiguities, resolved in code rather than in chat**, as `ASSUMPTION [CIRC-1..4]` — grep the tag to find all four. **CIRC-1:** `CIRCUMSTANCES_BY_CAUSE` is authored to be plausible, not authoritative; it stands in for BMPCC-18146's RDA circumstance ref-data + peril mapping, which does not exist yet. When 18146 lands, **replace the map wholesale, don't patch it**. **CIRC-2:** the AC says "filtered by CONFIRMED peril" (singular) but FNOL captures `causeOfLoss` as an array — read as a union over the selected perils, user still picks exactly one. **CIRC-3:** the fallback rule above. **CIRC-4:** stored claim-level, although BMPCC-18157 publishes `circumstance` on `claimSectionCreated/Updated`, which hints the real model may hang it off the section; sections *render* the claim's value instead of storing their own, so moving to per-section later is additive, not a migration.
  - **All the logic is in a pure config module, none of it in a component.** `step-loss-information.component.ts` (530 lines) and `claim-overview.component.ts` (440) are both already over the 300-line limit — putting the catalogue, the narrowing rule, or the label lookup in either would have made a known violation worse. `circumstances.ts` holds three pure functions (`circumstanceOptionsFor`, `isCircumstanceValidFor`, `circumstanceLabel`) with 13 unit tests and no TestBed; display resolution goes through `CircumstanceLabelPipe` (mirrors `DamageTypeLabelPipe`, same `'–'` for absent). The FNOL step grew ~20 lines, Claim Overview 2.
  - **Stale-pick correctness is the non-obvious part.** Re-pointing the cause of loss can leave a circumstance the new peril no longer offers. `onCauseOfLossChange` clears it via `isCircumstanceValidFor` on both FNOL and the Edit screen — and on the Edit screen that clear is what makes the loss show up as a row in the change ledger, so the user sees what their edit knocked out instead of it vanishing at save time.
  - **Labels, not keys, in the diff.** `computeLossInfoDiffs` compares `circumstanceLabel(...)` values, because that diff is read by a human in the confirm modal and `hot-work` tells them nothing. Deliberately **not** in `IMPACT_LABELS` — a more precise circumstance doesn't re-point any `ClaimSection.damageType`, so it must not trigger the sections-review redirect.
  - **`incidentCircumstance` is synced onto the overview from both write paths** — FNOL submit (`step-summary`) and Edit claim save (`syncOverviewFromLossInfo`) — for the same reason `dateOfLoss`/`proximateLossCause` are: Claim Overview and the section panel read the overview copy, not the `LossInformation` record.
  - **Mock data seeded deliberately incomplete.** 3 of 5 overview seeds carry a circumstance (`electrical-fault`, `hot-work`, `unknown`); `CLM-2024-011` and `CLM-2024-003` have none on purpose, so the `'–'` fallback for pre-existing claims is visible in the prototype rather than theoretical. 6 of 7 `loss-information.json` records seeded. Note the pre-existing inconsistency this exposed: that file uses cause keys (`water-damage`, `wind-storm`, `other`) that **do not exist** in `lookups.json` causeOfLoss — they fall through CIRC-3's unmapped branch, which is why those records show the full catalogue.
  - **Verification:** `ng build` clean; 56/56 tests pass (was 42 — 13 new pure tests + the existing suite); `audit:ac-logic` 58 passed / 0 failed; `pre-commit` 17/18. `audit:ndbx-wrapper`'s 3 hits are the same pre-existing/foreign ones as the last two entries (`navbar.html:21`, `file-restriction-card.component.html:18`, `add-section-entity-modal.component.html:99`) — none in a file this work touched.
  - **Committed as 18160-only hunks, not whole files.** The tree carried ~90 unrelated uncommitted files (the BMPCC-18352 rejection pilot, CBI/third-party work, tour `data-tour-id`s, BMPCC-15828 `policyVersion`) intersecting the same files — `claim-overview.component.{ts,html}`, `sections.ts`, `step-summary.component.ts`, `step-loss-information.component.html`, `claim-overview.model.ts`. Staged by filtering the diff to hunks that mention `circumstance` and `git apply --cached`, then verified the staged blobs reference none of the excluded identifiers. Per CLAUDE.md: uncommitted work gets swept into unrelated commits by accident, and that accident was one `git add <file>` away here.

---

## 2026-09-02 — Tracker: manager-only tickets (verbal brief, no ticket)

- **Source:** verbal brief — *"ใน Tracker มี isabelle ให้มี password ส่วนตัวแล้วจำด้วย เพราะเขาคือหัวหน้า ชื่อ isabelle คนอื่นจะ filter ไม่ได้ ticket ของ isabelle คนอื่นจะไม่เห็น"*, then *"password ใช้ isabelle ตัวเล็กหมด เริ่มตามแผนที่แนะนำ"*. Plan was presented and approved before any code.
- **Module:** Tracker (internal tool) + the `tracker-data/` dump + `TRACKER.md`
- **Files touched:** 13 — new `core/services/tracker-visibility.ts` + spec, `core/services/tracker-viewer.service.ts` + spec, `core/services/tracker.mappers.ts`, `features/tracker/tracker-owner-unlock/*.{ts,html,scss}`, `scripts/gen-tracker-md.mjs`; modified `core/services/tracker.service.ts`, `features/tracker/tracker-table/tracker-table.component.{ts,html,scss}`, `scripts/dump-tracker.mjs`, `TRACKER.md`, `tracker-data/*`.
- **Deviations:**
  - **This is a visibility rule, not a security boundary — said so up front and it was accepted.** The anon key ships in the bundle and RLS on every tracker table is `to anon, authenticated using (true)` (`supabase/migrations/0002_tracker_remove_auth_requirement.sql`), so anyone who opens DevTools can query the hidden rows directly. Making it real needs Supabase Auth + per-user RLS, which re-imposes sign-in on the whole team — removed on request on 2026-08-20. Level A (app-level hide) was chosen over Level B (real RLS) for that reason. The trade-off is written into the header of `tracker-visibility.ts` so the next reader doesn't mistake it for enforcement.
  - **Password is compared as a SHA-256 hex constant in the client**, same pattern as the existing `features/access-gate` gate (`core/services/auth.ts:36-56`). Consistent with the above: it keeps her rows off a colleague's screen, it does not withstand someone reading the bundle.
- **Schema changes:** none. No migration, no new column — the rule is derived from `ticket.assignee` matching `isabelle`.
- **Notes:**
  - **The PostgREST NULL trap is the whole reason this needed a spec.** `assignee=not.ilike.*isabelle*` also drops every *unassigned* ticket, because `NULL ILIKE '%x%'` is NULL and `NOT NULL` is still NULL. Measured live before writing any code: 57 active rows → 45 instead of 51. Correct form is `or=(assignee.is.null,assignee.not.ilike.*isabelle*)` → 51. Locked in by `tracker-visibility.spec.ts`, which also asserts the filter string contains no `%` — PostgREST's LIKE wildcard is `*`, and a `%` there fails silently by matching nothing.
  - **Four leak paths, not one.** (1) the list — filtered server-side in `getTickets()`; (2) the Assignee dropdown — cleans itself for free, it's `computed()` over whatever loaded, so her name is simply absent from the options; (3) **`?key=BMPCC-14833` — the real hole**, since `ticket-detail-panel` fetches by key independently of the list, so hiding table rows alone left every hidden ticket one shared URL away; fixed by applying the same filter in `getTicket()`; (4) notes/relations — covered transitively, the panel's `load()` returns early when `getTicket()` yields null.
  - **A fifth found while building:** locking while one of her tickets was open left the already-fetched row on screen, because the panel only refetches on `jiraKey` change. Closed by a `previousViewer`-guarded effect in `tracker-table.component.ts` — guarded rather than firing on first run, or a deep link would be cleared on page load.
  - **`.maybeSingle()` instead of `.single()` in `getTicket()`** so a withheld key yields `(null, null)` and the panel shows "This ticket could not be found." A `single()` there raises PGRST116, and a distinct error for a hidden row versus an absent one tells the reader the row exists.
  - **The repo was leaking what the app hid.** `TRACKER.md` listed all 6 of her tickets by name and `tracker-data/` held their notes. `dump-tracker.mjs` now fetches everything then withholds her rows from `ticket`/`ticket_state`/`note`/`relation`, and `_dump-summary.json` asserts `rows + withheld_owner_rows === reported_total` so a withheld row can never be mistaken for a failed fetch. `INCLUDE_OWNER=1` produces the full dump locally and prints a warning; do not commit that output. The throwaway TRACKER.md generator was promoted to `scripts/gen-tracker-md.mjs` and now reads `tracker-data/*.json` instead of curling Supabase, so the doc inherits the withholding structurally rather than by remembering to filter.
  - **Two files still name her and were deliberately left alone:** this file and `TECH_DEBT.md`. Both are append-only history; rewriting them to erase a name would falsify the record for no gain, since neither lists her tickets.
  - **`tracker.service.ts` was at 322 lines, over the 300 limit, before any of this.** The seven row-mapper functions were moved verbatim into `tracker.mappers.ts` first (no behaviour change), bringing it to 242, and only then was the visibility wiring added.
  - **Verification:** 69/69 tests pass (8 files, 13 new); `ng build` clean; `pre-commit` 17/18 — `audit:ndbx-wrapper`'s 3 hits are the same pre-existing/foreign ones as the previous entries, none in a file this work touched. Checked against the live project with a real client: as team 51 rows / 0 hers / 6 unassigned still present, as owner 57 / 6 hers; `BMPCC-14833` fetched by key as team → `data=null err=null`; a visible key still resolves; the joined `ticket_state` for a hidden key → `[]`.
  - **Committed separately from the tracker fixes it sat on top of.** The tree already carried an uncommitted batch of earlier tracker work (`?key=` route, split loading flags, request-sequence guards, sync refreshing the list, add-note error handling) touching two of the same files. Saved this work aside, reverted to expose the pre-existing state, committed that as `f353d14`, then replayed — rather than sweeping someone else's unreviewed batch into a commit titled "manager access".
...
## 2026-09-02 — Tracker links: one column removed, new tab, and an arrival panel

- **Source:** verbal brief — *"เอา design ออกแล้วเหลือแค่ build กับ hand off เวลากด link ก็ new tab … แล้วไหนทัวร์วะ พอลิ้งไปแล้วไงก็ไม่บอกอะไรเลย เหมือนพาคนไปยืนกลาง Times Square แล้วมึงหาทางเอาเองนะ … อย่า hardcode tour"*
- **Module:** Tracker (internal tool) + a new app-root overlay
- **Commits:** `1c14712` (Design column removed), `95681fe` (`:id` substitution + new tab), `4c60d64` (arrival panel + `audit:prototype-links`)
- **Files touched:** 18 in the last commit; `tracker.model.ts`, `tracker-table.component.{html,scss,ts}`, `ticket-detail-panel.component.ts`, `app.ts`, `app.config.ts`, `prototype-{scenario,entry}.service.ts`, `tracker.service.ts`, `tour.service.ts`, `claim-dev-helper.service.ts`, new `core/models/arrival-context.model.ts`, `core/services/arrival-context.builder.ts`, `shared/components/arrival-panel/*`, `scripts/{gen-tracker-md,dump-tracker,audit-prototype-links}.mjs`, `supabase/migrations/0001_tracker.sql`, `TRACKER.md`
- **Deviations:** the arrival panel sits at `z-index: 1200`, between the toast stack (1100) and the tour overlay (`--tour-overlay-z`, 5000), so a running tour's highlight ring is never covered by the panel that launched it. Bottom-left, because top-right is the toast stack's and dead centre is the tour card's.
- **Schema changes:** `TicketState.designStatus` dropped from the model. The `design_status` DB column stays, marked LEGACY in the migration — dropping a column to remove a UI element is not a trade worth making.
- **Notes:**
  - **`/claims/:id/sections` navigated LITERALLY matches the parameterised route with the id `":id"`.** The tracker's `prototype_route` is hand-typed, copied out of `app.routes.ts` param and all, and `resolveRoute()` returned it verbatim. Proven over CDP: `/claims/:id/loss-information/edit` renders "Edit claim details" with `:id` printed where the claim number goes and 6× "Not provided"; the same route with `CLM-2024-001` renders the claim and 0× "Not provided". **The data was never lost, and 17 of the 30 clickable rows were doing this.** Fixed at one point — `substituteClaimId()` + `DEMO_CLAIM_ID` in `prototype-scenario.service.ts` — preferring the linked ticket's own `targetClaim`. 46 of the stored routes carry `:id`; 0 survive substitution.
  - **A new tab cannot inherit `sessionStorage`, so the URL is the only channel.** `?pt=<ticketId>` applies the scenario, `?tk=<jiraKey>` fetches the row. `openRoute()` (apply→navigate→postLand in the *tracker's* tab) is gone; the sequence now runs in the tab that receives the link. Side effect worth having: the links are shareable — paste one anywhere and it opens with the scenario applied.
  - **`provideAppInitializer` runs before `provideRouter`'s initial navigation**, which is the only reason the target page's *first* render already sees the seeded state instead of repainting off signal propagation. The second half has to wait for the access gate, because both the tour renderer and the panel live inside `AppComponent`'s `@if (unlocked())`.
  - **`App.ngOnInit` runs during bootstrap, where `router.url` is still `'/'`.** The first version read the url there and fired a second navigation that cancelled the router's own. It now awaits the first `NavigationEnd`.
  - **Nothing in the panel is authored per ticket, and there is nowhere to author it.** `ArrivalContext` has no "orientation text" field; every line comes from `TicketAC.statement`/`expectedUI`/`howToTest` or the tracker row. Measured: 8 of 30 clickable rows have a ticket file, 22 have nothing but the row — so the honest empty state is the COMMON branch, not the edge case, and it says what is and isn't known rather than pretending to orient anybody.
  - **The tour is offered, not auto-started.** Landing mid-tour with a popover already over the screen is the same "stand here and figure it out" in a different costume. A declared `postLand` still auto-runs, because that IS the scenario (it opens the modal the criterion is about) rather than narration of it.
  - **`buildFallbackTour()` fires for none of the 24 current ticket files** — all of them have `walkthroughSteps`. It is kept so that adding a ticket file without hand-writing a walkthrough yields a working tour instead of a silently missing one, and `audit:prototype-links` asserts the invariant that matters (every ticket file resolves to a non-empty tour through one path or the other). Claiming it was exercised would have been false.
  - **The panel must name the screen you are on, not the one you landed on.** `BMPCC-17157` lands on `/fnol/skeleton-parties` and the wizard immediately moves to `/fnol/skeleton-create`; a frozen landing route stated the wrong location while the reviewer looked at another page. Found by the verification run, not by reading the code.
  - **Only 7 distinct design/build/handoff combinations existed across 51 active rows, 15/15 handoff-done rows were also build-done, and exactly one row (`BMPCC-18353`) was build-ahead-of-design.** A column with that little independent signal is a column that costs width and explains nothing. Freed 150px, handed to `.col-title` (320 → 470px when grouped).
  - **The launch button now says what the click gives you, before the click:** 8 in `--interactive-primary` with "N acceptance criteria explained on arrival", 22 muted with "no acceptance criteria recorded". Verified 22/8 in the browser — matches the measured split exactly.
  - **`audit:prototype-links` (new, in `pre-commit`)** checks every AC route against every path declared in `app.routes.ts`/`fnol.routes.ts`/`tracker.routes.ts`, rejects a surviving route param, requires a non-empty resolvable tour per ticket file, and checks `DEMO_CLAIM_ID`/`substituteClaimId` parity between the TS and `gen-tracker-md.mjs` (which duplicate it by hand, because a node script can't import TS). **Negative-tested before trusting it** — a typo'd route and a re-introduced `/claims/:id/limits` each fail it, and the ticket file was restored byte-for-byte afterwards. An audit that has never been seen to fail is not evidence.
  - **Verification:** CDP against `localhost:4200`, 5 rows launched → 5 new tabs. `BMPCC-14629` → panel + 5 expandable criteria + "Start tour · 11 steps" which runs and collapses the panel to a pill. `BMPCC-14840`/`18353`/`18370`/`17157` → panel + empty state with the correct title and Build/Handoff chips read from Supabase. No literal `:id` in any path, 0× "Not provided", 0 console errors in any tab. Table geometry: 0px overflow and 0 clipped cells in both grouped and flat mode at 1440px. `ng build` clean; `pre-commit` 18/19 — the one failure is `audit:ndbx-wrapper`'s 3 pre-existing lines in `add-section-entity-modal`, `navbar` and `file-restriction-card`, none touched here.
...
## 2026-09-03 — Matching the production repo's detail: Loss Event Overview, and damaged items

- **Source:** verbal brief — *"เติมความละเอียดของเราให้เท่ากันกับเขา"*, after a screen-by-screen comparison against the real `claims-management` repo's MFEs
- **Module:** Loss Event Overview (rebuilt), Sections → damaged items, `page-shell` (shared)
- **Files touched:** 20 across two commits — new `core/models/loss-event-overview.model.ts`, `core/mock/services/mock-loss-event.service.{ts,spec.ts}`, `features/loss-events/loss-event-general-info/*`; rewritten `features/loss-events/loss-event-overview/{ts,html,scss}`; `shared/components/page-shell/page-shell.component.html`; `features/sections/damaged-item.config.ts`, `{add,edit}-damaged-item-modal/*`, `entity-detail-panel/{ts,html}`; `core/mock/data/{loss-events,claims}.json`; `core/mock/services/mock-lookup.service.ts`; both barrels
- **Schema changes:** `DamagedItem.financialLossCausedBy` → `causedBy` (see below). `LossEventDetail` = `LossEventSummary` + the five new general-info fields on each `loss-events.json` record. `loss-events.json` gained a 7th event (`111213`).
- **Deviations:** upstream renders its Financial overview and Documents tabs as placeholder paragraphs ("… will be displayed here when available") and hosts Parties as an embedded `wc-cc-party-embedded` web component. All three are real, data-fed tables here — copying a placeholder to match a placeholder adds no detail.
- **Notes:**
  - **Three of five claimed gaps were not gaps, and were dropped after checking our own code rather than the comparison notes.** Retention period: we already offer 3 options where upstream offers 2. Impacted policies: fully built here. The sections Comments column and the material-damage Documents column upstream are an empty `<td>` and a hardcoded `'0'` — there is nothing behind either to match. **The comparison was written from their templates; two of its findings were wrong, and both were reported to the user as wrong.**
  - **`page-shell` had never once projected content in `align="grid"` mode, and nothing anywhere said so.** The template declared a default `<ng-content>` in BOTH branches of `@if (align === 'grid') … @else`. Angular projects the caller's content into ONE slot, and it took the `@else` one — so the grid branch rendered an empty `nxCol="12"` with zero console errors. It went unnoticed because the only existing caller (`administration.component.html`) passes `align="navbar"`, while `BLESSED.md` documents grid as the default. Fixed with a single `<ng-template #shellContent><ng-content></ng-content></ng-template>` stamped by `[ngTemplateOutlet]` into each branch. **Found by taking a screenshot, not by reading the diff** — the symptom was a navbar and a breadcrumb over 2000px of white, with not even the "Loading loss event…" line.
  - **The first hypothesis was wrong and cost a round trip:** that projecting a bare `@if` control-flow block breaks projection. Wrapping the content in a `<div class="leo-content">` changed nothing, and the DOM dump then proved the wrapper was absent from the document entirely — which is what pointed at the slot rather than the content.
  - **`CLM-2024-005` pointed at loss event `111213`, which had no record in `loss-events.json`.** A dead link, in the tree, committed. The event now exists (Machinery Breakdown — BMW Budapest Motors), and every event's `claimCount` was reconciled against the claims that actually reference it — 21 claims now carry a `lossEventId`, 0 dangling. Both invariants are gated by `mock-loss-event.service.spec.ts` so the next data edit fails a test instead of blanking a table.
  - **Reported loss is rolled up per currency, never summed.** Event `345678`'s claims are booked in mixed currencies; one total would be a fabricated number. An `nx-message` on the tab says why, so the absence of a single figure reads as a decision rather than a missing feature.
  - **`parties.json` keys claims as `CL-2024-005` while `claims.json` uses `CLM-`.** Normalised inside `collectParties()` rather than renumbering the file, because the party tree and `risk-analyses.json` read the same records. Without it the Parties table is silently empty — hence its own test.
  - **General info is read from the raw JSON, not from `MockStateService`.** State types its copy as `LossEventSummary`, which drops the five general-info fields. The summary still comes from state so scenario overrides keep applying; only the block state doesn't carry is read around it.
  - **`financialLossCausedBy` became `causedBy` because the field was never financial-loss-specific upstream** — every damage type asks it. That also promoted it from a sub-line under Description to its own table column: a sub-line was defensible while one damage type in four had a value, and misleading once every row does.
  - **The Bodily injury branch keeps the injured person as a sub-line, not three columns.** Only bodily-injury rows have one, and the table lives inside a side panel — three columns empty on most rows costs width that the panel doesn't have.
  - **`DamagedItem` stayed feature-local in `sections/damaged-item.config.ts`.** `core/models/reserve.model.ts` already exports an unrelated `DamagedItem` keyed by `damagedItemId`; moving ours next to it would have made two different shapes share one name in one barrel.
  - **Carried along, not built here:** `mock-lookup.service.ts` also contains an earlier session's `getCbiCaseTypesSync()`, and the second commit carries `lookup.model.ts` / `section.model.ts` / `cbi-case-type-label.pipe.ts` from the BMPCC-18353 CBI pilot, because `entity-detail-panel`'s template will not type-check without them. `claims.json` likewise carries ~10 lines of an earlier session's assignee/date edits. Named here because a commit that sweeps in unrelated work silently is the thing RULE -2 exists to prevent.
  - **Open defect belonging to that CBI work, reported not fixed:** `lookups.json` has no `cbiCaseTypes` key, so `getCbiCaseTypesSync()` returns `undefined` and `add-section-entity-modal`'s CBI case-type `@for` iterates it. `Lookups` is cast `as unknown as`, so `tsc` says nothing. Harmless in the panel (the pipe returns `–` before touching the array) and unreachable until a CBI entity is added.
  - **Verification:** CDP against `localhost:4200`, 0 console errors. `/loss-events/123456/overview` renders the title row + Monitoring chip, three real horizontal tabs (`role="tablist"`, 3 `nx-tab-header__item`, measured at y=254 side by side — an earlier stale screenshot showing an accordion was from a previous iteration of the page), General information with all four fields + 3 causes + 2 damage groups, Related claims with CLM-2024-001/011/102 and their handlers, Parties on its empty state, Financial overview at EUR 940,000 reserve + the per-currency row, Documents with 3 files sized 471 KB / 1 MB / 3 MB. `/administration` still renders, so the shared `page-shell` fix didn't regress the only other caller. `npx tsc --noEmit` clean; `ng build` clean; `ng test` 78/78 including 8 new `MockLossEventService` gates; `audit:ac-logic` 81 ACs / 58 passed / 0 failed; `pre-commit` 18/19 — the one failure is `audit:ndbx-wrapper`'s 3 pre-existing lines, none of them in a file touched here.
